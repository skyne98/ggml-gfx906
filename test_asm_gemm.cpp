...
int main() {
    ...
    printf("=== GFX906 Inline Assembly GEMM Test ===\n");
    printf("Matrix size: %dx%dx%d\n", M, N, K);
    printf("Iterations: %d\n\n", iterations);
    
    // Test FP16
    {
        printf("Testing FP16 with inline assembly...\n");
    ...
    // Test FP32
    {
        printf("Testing FP32 with inline assembly...\n");
        ...
        // Warmup
        for (int i = 0; i < 5; i++) {
            gemm_f32_gfx906_asm<<<grid, block>>>(d_A, d_B, d_C, M, N, K, 1.0f, 0.0f);
        }
        HIP_CHECK(hipDeviceSynchronize());
